...
function fixFlowchartSyntax(code: string): string {
  ...
  const getOrCreateId = (label: string): string => {
  ...
  return code
    ...
    .replace(/(?<![A-Za-z0-9_])\(\(([^)]+)\)\)/g, (_, label) => `${getOrCreateId(label)}((${label}))`)
    .replace(/(?<![A-Za-z0-9_])\[([^\]]+)\]/g, (_, label) => `${getOrCreateId(label)}[${label}]`)
    .replace(/(?<![A-Za-z0-9_(])\(([^)]+)\)/g, (_, label) => `${getOrCreateId(label)}(${label})`)
    .replace(/(?<![A-Za-z0-9_])\{([^}]+)\}/g, (_, label) => `${getOrCreateId(label)}{${label}}`);
}
...
export default function FlowchartRenderer({ content }: FlowchartRendererProps) {
  ...
  const code = fixFlowchartSyntax(extractMermaidCode(content));

  const handleDownload = async () => {
    setLoading(true);
    setError("");
    try {
      await downloadAsPdf(code, "Flowchart");
    } catch {
  ...
  return (
    <div className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-lg p-10 text-center">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
        Your flowchart has been generated. Download it as a PDF to view or share it.
      </p>
